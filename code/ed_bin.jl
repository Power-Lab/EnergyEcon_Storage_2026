...
function ed_bin(gen_df, loads, gen_variable, self_schedule=false, storage_quantity=nothing, discharge_bin_fixed=false, discharge_bin=nothing)
    
    # Initialize the model
    ED = Model(Gurobi.Optimizer)
    if !discharge_bin_fixed
    ...
    if !self_schedule
        if !discharge_bin_fixed
            ...
            @constraint(ED, cCapCharge[i in G_stor, t in T], 
                CHAR[i,t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 1 * (1 - discharge_bin[t]))
            @constraint(ED, cCapDischarge[i in G_stor, t in T], 
                DISC[i,t] <= gen_df[gen_df.r_id .== i,:existing_cap_mw][1] * 1 * discharge_bin[t])
        end
    else
    ...
    # Obtain storage operational results
    if !discharge_bin_fixed
        discharge_bin_df = DataFrame(hour=Array(T), discharge_bin=vec(sum(Array(value.(DISC_STAT)), dims=1)))
    else
        discharge_bin_df = DataFrame(hour=Array(T), discharge_bin=Array(discharge_bin))
    end
