#=
A bi-level model that has been converted to single level with demand response
Inputs:
...
function sbed_cvx_dr(gen_df, loads, gen_variable)
    ...
    @variables(SBED_CVX, begin
        ...
        CHAR_PRICE_OFFER[T] >= 0  # in $/MWh
        DISC_PRICE_OFFER[T] >= 0  # in $/MWh
    ...
    @constraint(SBED_CVX, cDemand[t in T], 
        ...
        sum(DISC_CLEARED[i,t] for i in G_stor) == (loads[loads.hour .== t,:demand][1] - DR[t])
        )

    @constraint(SBED_CVX, cDR[t in T],
        DR[t] <=  loads[loads.hour .== t,:demand][1] * 0.05 )
    
    # KKT: dual feasibility
    # (6a): L with respect to cleared charge power
    @constraint(SBED_CVX, cDF1b[t in T],
        (LAMBDA[t]) <= 1000 )
    @constraint(SBED_CVX, cDF1a[t in T],
        (CHAR_PRICE_OFFER[t] - LAMBDA[t]) <= 0 )
    ...
    @constraint(SBED_CVX, cDF2a[t in T],
        (DISC_PRICE_OFFER[t] - LAMBDA[t]) <= 0 )
